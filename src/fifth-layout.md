...
> Возможно, это было самое сложное расширение языка со времён *борроучекера*.
> Но нам, в любом случае, не надо закреплять наш список!
> 
> Закрепления нужны и полезны для async/await, футур, корутин, поскольку компилятору надо собирать локальные переменные функции в некоторое подобие структуры и хранить их где-то, пока футура/корутина не будет готова к возобновлению.
