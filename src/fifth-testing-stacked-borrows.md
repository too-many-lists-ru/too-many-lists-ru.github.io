...
> That was a lot of theory and ideas -- let's move on to the true heart and soul of this book: riting some bad code and getting our tools to scream at us. We're going to go through a *ton* of examples to try to see if our mental model makes sense, and to try to get an intuitive feel for stacked borrows.
...
> Rust allows borrows to be broken up so that they only apply to particular parts of the allocation! Let's try it out:
...
> A line like `let sref2 = &*mref1` is a tricksy thing. *Syntactically* it looks like we're dereferencing it, but dereferencing on it's own isn't actually a *thing*?
